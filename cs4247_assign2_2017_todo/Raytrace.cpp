...
// Compute shadow if hasShadow is true.
	...
	for (int i = 0; i < scene.numPtLights; i++) {
		Vector3d pointLightPos = scene.ptLight[i].position;
		Vector3d L = pointLightPos - nearestHitRec.p;
		...
		if (hasShadow) {
			for (int j = 0; j < scene.numSurfaces; j++) {
				...
				if (scene.surfacep[j]->shadowHit(shadowRay, DEFAULT_TMIN, tmax)) {
					isInShadow = true;
					break;
				}
			}
		...
		if (!isInShadow){
			Color phong = computePhongLighting(L, N, V, *nearestHitRec.mat_ptr, scene.ptLight[i]);
			result += phong;
		}
...
// Add to result the reflection of the scene.
	...
	for (int i = 0; i < reflectLevels; i++) {
		Vector3d reflectDirection = mirrorReflect(V, N);
		Ray reflectRay(nearestHitRec.p, reflectDirection);
		return  result + nearestHitRec.mat_ptr->k_rg * TraceRay(reflectRay, scene, reflectLevels - 1, hasShadow);
	}
